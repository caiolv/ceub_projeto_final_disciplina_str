...
function round_robin(processos, quantum, qnt_processos) {
    ...
    let bt_restante = Array.from({length: qnt_processos}, (_, i) => 0);
    // Criando uma lista de Waiting Time
    let wt = Array.from({length: qnt_processos}, (_, i) => 0);
...
function turn_around_time(processos, wt, qnt_processos) { 
    let tat = Array.from({length: qnt_processos}, (_, i) => 0);
    for (var i = 0; i < qnt_processos; i++)
...
function average_tat(tat, qnt_processos) { 
    let turnaround_time = tat.reduce(reducer);
    return (turnaround_time/qnt_processos)
}

function average_wt(wt, qnt_processos) { 
    let waiting_time = wt.reduce(reducer);
...
const qtdProcessos = inputInt("Quantidade de processos: ")
...
const quantum = inputInt("Informe o Quantum: ")
...
//Waiting Time
const wt = round_robin(processos, quantum, qtdProcessos)
//TurnAround Time
const tat = turn_around_time(processos, wt, qtdProcessos)
//Média de todos os TurnAround Time
const avg_tat = average_tat(tat, qtdProcessos)
//Média de todos os Waiting Time
const avg_wt = average_wt(wt, qtdProcessos)
console.log(`WT = ${wt}\nTAT = ${tat}\nAVG_TAT = ${avg_tat}\nAVG_WT = ${avg_wt}`)


console.log("| Process |\t| Burst Time |\t\t| Arrival Time |\t| Waiting Time |\t| Turn-Around Time |\t\n\n")
for (proc = 0; proc < processos.length; proc++)
    console.log(`${processos[proc][0] + proc}\t\t\t${processos[proc][2]}\t\t\t${processos[proc][1]}\t\t\t${wt[proc]}\t\t\t         ${tat[proc]}\t\t\t\n`)
